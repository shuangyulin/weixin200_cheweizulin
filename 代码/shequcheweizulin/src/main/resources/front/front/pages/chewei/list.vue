<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.cheweiName" type="text" placeholder="车位地址"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in cheweiTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
              :style='{"fontSize":"26rpx","lineHeight":"40rpx","color":"#333","textAlign":"left"}'>
            {{product.cheweiName}}</view>
        ...
        <view class="uni-product-price" v-if="product.cheweiNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
            <text class="">￥{{product.cheweiNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('chewei','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('chewei','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('chewei','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 车位类型 字段 字典表查询方法
		  	let cheweiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'chewei_types'
		  	});
		  	this.cheweiTypesList = cheweiTypes.data.list;
		...
		methods: {
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.cheweiTypes = this.categoryName
                            }

                let res = await this.$api.list(`chewei`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('chewei', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.cheweiName) {
                    searchForm['cheweiName'] = this.searchForm.cheweiName
                }
				if (this.searchForm.cheweiName) {
					searchForm['cheweiName'] = this.searchForm.cheweiName
				}

                let res = await this.$api.list(`chewei`, searchForm);
				// 如果是第一页数据置空
