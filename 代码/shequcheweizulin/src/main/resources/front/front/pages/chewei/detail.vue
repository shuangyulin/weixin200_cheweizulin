...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
			...
			<!-- 名字-->
						<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.cheweiName}}
						</view>
				</view>
   	 <view style="margin-top: 10rpx;"></view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.cheweiContent"></rich-text>
				</view>
			</view>
            ...
            <view class="bottom-content cu-bar bg-white tabbar border shop">
                ...
                <view style="text-align: right;">
					...
					<button @tap="onSubmit" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur"
                            :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3],borderColor:btnColor[3]}">立即租赁</button>
                </view>
            </view>
		</view>
...
<script>
	export default {
		...
		methods: {
			async onSubmit(){
				 this.$utils.jump(`../cheweishenqing/add-or-update?cheweiId=`+this.id)
			},
		    //下载
            download(url) {
                let _this = this;
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('chewei', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.cheweiPhoto ? this.detail.cheweiPhoto.split(",") : [];
			},
